...
lsTree = ET.parse(base+'RegionConfigFiles/LocationSets 3.50.xml')
...
for set in lsTree.findall(".//{http://www.wldelft.nl/fews}locationSet[@id='Gages']/{http://www.wldelft.nl/fews}locationSetId"):
  [tag, basin] = set.text.split("_")
  print basin
  ...
  for set2 in lsTree.findall(findText):
    ...
    try:
      dupFile(base+'ModuleConfigFiles/palp1/ARMA_PALP1_PALP1_Forecast 3.00.xml',\
          base+'ModuleConfigFiles/'+idDict[loc].lower()+'/ARMA_'+idDict[loc]+'_'+loc+'_Forecast.xml','PALP1',loc)
    except:
      continue
    try:
      dupFile(base+'ModuleConfigFiles/palp1/MERGETS_ARMA_PALP1_PALP1.xml',\
          base+'ModuleConfigFiles/'+idDict[loc].lower()+'/MERGETS_ARMA_'+idDict[loc]+'_'+loc+'.xml','PALP1',loc)
    except:
      continue
